...
일단, **AES-128, AES-192, AES-256**이라는 단어를 많이 들어봤을 텐데 그것은 키의 길이로 결정된다.<br> 3종류의 키를 사용할 수 있는데 라운드 함수 또한, 128bit 키 사용시에는 10라운드, 192bit에서는 12라운드, 256bit에서는 14라운드를 실행한다.<br>
...
+ **IV(Initialization Vector)**<br>
   ...
   특히, 내가 코드로 구현한 모드는 CBC이다.<br>
   CBC 모드에서는 IV는 암호화시 예측할 수 없어야 하는데 메시지의 마지막 암호문 블록을 다음 메시지의 IV로 다시 사용하는 방법은 안전하지가 않다! 왜냐하면, 공격자가 다음 일반 텍스트를 지정하기 전에 IV를 알고 있으며, 이전에 동일한 키로 암호화된 일부 블록의 일반 텍스트에 대한 추측할 수 있으며 공격으로 이어지기 때문이다.(TLS CBC IV Attack의 방식)[^13]  
